      ...
      <div class="login-box">
         <div class="card card-outline card-primary" style="border-top: 3px solid #4CB648;">
            <div class="card-header text-center">
               ...
               <a href="<?= base_url('app'); ?>" class="p"><b><?= $this->general_settings['application_name']; ?></b>Master Login</a>
            </div>
            <!-- /.login-logo -->
            <div class="card-body">
               ...
               <div class="input-group mb-3">
                  <input type="number" name="contact" id="contact" class="form-control" placeholder="Enter mobile no." <?php if($this->session->userdata('is_contact_verify') === true){ echo 'value="'.$this->session->userdata('contact').'" readonly';}?>>
               ...
               <p id="change_number" <?php if($this->session->userdata('is_contact_verify') !== true){ echo 'style="display: none;text-align: right;font-size: 13px;text-decoration: underline;margin-top: -16px;margin-bottom: 6px;"';} else { echo 'style="text-align: right;font-size: 13px;text-decoration: underline;margin-top: -16px;margin-bottom: 6px;"';}?>><a href="<?php echo base_url().'app/auth/change_number'; ?>" style="cursor: pointer;" >Change number</a></p>
               <div class="input-group mb-3 verify_otp" <?php if($this->session->userdata('is_contact_verify') !== true){ echo 'style="display: none;"';}?>>
                  <input type="number" name="otp" id="otp" class="form-control" placeholder="Enter otp">
               </div>
               <div class="row pb-3 pt-2">
                  <div class="col-12 verify_contact_btn" <?php if($this->session->userdata('is_contact_verify') === true){ echo 'style="display: none;"';}?>>
                  ...
                  <div class="col-12 verify_otp_btn" <?php if($this->session->userdata('is_contact_verify') !== true){ echo 'style="display: none;"';}?> >
      ...
      <script type="text/javascript">
         function verify_contact(){
            if($("#contact").val()!=""){
               $.ajax({
                 type: "POST",
                 url: baseURL+'app/auth/verify_contact',
         ...
         function verify_otp(){
            if($("#otp").val()!=""){
               $.ajax({
                 type: "POST",
                 url: baseURL+'app/auth/verify_otp',
                 data: {otp:$("#otp").val()},
                 dataType: "json", 
                 success: function(ajax_response2){
                    var newData2 = JSON.stringify(ajax_response2)
                      ...
                      if(obj2.status == true){
                        location.replace(baseURL+'users');
                      } else {
